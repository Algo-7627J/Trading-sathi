...
st.title("🤖 Trading Sathi - Intraday Scanner + Next-Day Outlook")
st.caption("Technical + Momentum + Volume + Multi-candle Patterns + OI + News + Fundamentals + Sector View + Multi-Timeframe Confirmation + Backtested Next-Day Outlook")
...
if st.session_state.fyers is None:
    ...
    with tab_intraday:
        ...
        a, b = st.columns(2)
        with a:
            if st.button("🔍 Run Smart Scan", type="primary", key="run_intraday_scan"):
    ...
    with tab_next_day:
        st.warning(
            "⚠️ **Please read before using**: No model can reliably predict "
            "tomorrow's exact move - markets are affected by news, global "
            "cues and randomness no technical signal can see. This tool "
            "combines several daily-timeframe signals AND backtests that "
            "exact rule-set on each stock's own past ~1 year of data, so "
            "every call shows an honest historical hit-rate instead of a "
            "blind score. Treat 'STRONG BULLISH/BEARISH' with HIGH "
        ...
        st.write(f"Symbols queued for analysis: **{len(nd_symbols)}**")
        st.caption(

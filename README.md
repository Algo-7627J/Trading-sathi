...
- **🇮🇳 Jaipur INR rates**: today's **Jaipur bullion rates in rupees** on the Gold & Silver tab — 24K & 22K gold per 10g, silver per kg, with the day's change and a 7-day trend (GoodReturns city pages; COMEX × USD/INR fallback)
...
- **💬 Social Buzz (new)**: "what people are saying" as move-triggers — Reddit posts from the **last 7 days** across Indian trading subreddits (r/IndianStreetBets, r/IndiaInvestments, …) + Google News headlines where the stock was discussed on twitter/social media. **Smart ticker disambiguation** (ACC → "ACC Ltd" cement, not the US football conference; IOC → Indian Oil, not the Olympics) + a business-context relevance filter keep results about the *stock*. Shown as a compact buzz strip with a bullish/bearish tally on Strong Direction & Streak cards, plus a dedicated **💬 Social Buzz tab** (search any symbol, trending r/IndianStreetBets feed, CSV export). Purely informational — never changes scan scores. Optional: set `X_BEARER_TOKEN` in Streamlit secrets to add real X (Twitter) search.
